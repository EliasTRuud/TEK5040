...
def plot_filters(filters,op_file, max_filters_per_image=16):
    ...
    for im in image:
        plt.imshow(im)
        plt.imsave(op_file, im)
        plt.show()

def show_conv_layer(layer,op_file):
    ...
    plot_filters(filters,op_file)

def show_dense_layer(layer,op_file):
    ...
    plot_filters(kernel,op_file)

def plot_weights(agent, op_file):
    ...
    if has_conv_layers:
        show_conv_layer(agent.feature_extractor.layers[0], op_file)
    elif has_hidden_dense:
        show_dense_layer(agent.feature_extrator.layers[0],op_file)
    else:
        print("Showing weights for linear policy.")
        # TODO: show legend with action???
        show_dense_layer(agent.layers[-1], op_file)
...
def main(policy):
    ...
    op_file =str(Path(policy).parent)+'/filter_weights.png'
    plot_weights(agent, op_file)

def parse_args():
    """Parse command line argument."""

    parser = argparse.ArgumentParser(
        "Show weights for policy network trained on 'Car-Race-V2'."
    )
